...
export default class MapEditorPlacesPannel extends React.Component {
  ...
  renderTypeDropdown( place ) {
    console.log('renderTypeDrop',place.type)
    ...
    for( var i=0; i<PLACES_TYPES.length; i++ ){
      ...
      dropdownItemsArr.push(
        <DropdownItem as="button" value={PLACES_TYPES[i]} onClick={this.onTypeSelect.bind(this, place)} key={'typedropitem_'+i+'_'+place.id}>{typeName}</DropdownItem>
      )
    }
    ...
    return(
      <UncontrolledDropdown>
        <ButtonGroup dir="ltr">
          <DropdownToggle color="success" className="icon icon--right">
  ...
  onSubTypeSelect = (place,  event) => {
    this.props.onPlaceSubType( place, event.target.value )
  }

  onAmenityGroupByCheck = (place, status) => {
    console.log('-- status : ', status)
    this.props.onAmenityGroupByCheckChange(place, status);
  }


  renderSubTypeDropdown( place ) {
    let dropdownItemsArr = [];
    for( var i=0; i<AMENITIES_TYPES.length; i++ ){
      let typeName = AMENITIES_TYPES[i].charAt(0).toUpperCase() + AMENITIES_TYPES[i].slice(1);
      dropdownItemsArr.push(
          <DropdownItem as="button" value={AMENITIES_TYPES[i]} onClick={this.onSubTypeSelect.bind(this, place)} key={'typedropitem_'+i+'_'+place.id}>{typeName}</DropdownItem>
      )
    }

    let placeTypeName = ''
    if(place.subType != undefined){
      placeTypeName = place.subType.charAt(0).toUpperCase() + place.subType.slice(1);
    }else{
      placeTypeName = AMENITIES_TYPES[0].charAt(0).toUpperCase() + AMENITIES_TYPES[0].slice(1)
    }
    return(
        <UncontrolledDropdown style={styles.subTypeContainer}>
  ...
  renderConnectionDropdown( place ) {
    ...
    for( var i=0; i<linkConnections.length; i++ ){
      ...
      dropdownItemsArr.push(
        <DropdownItem as="button" value={linkConnections[i].id} onClick={this.onConnectionSelect.bind(this, place)} key={'connectiondropitem_'+i+'_'+place.id}>{connectionName}</DropdownItem>
      )
            ...
            <AccordionCollapseComponent
              ...
              >
                ...
                <span style={styles.placesElemHeaderLinkStatus}>
                    <Switch onChange={this.onPlaceStatus.bind(this, place)} checked={place.status} height={16} width={32} checkedIcon={false} uncheckedIcon={false}/>
                  </span>
              </div>}>
              ...
              <form className="form" style={styles.addModal}>
                ...
                <div className="d-block">
                   ...
                   <span style={styles.placesElemHeaderLinkStatus}>
                      <Switch onChange={this.onAmenityGroupByCheck.bind(this, place)}
                              checked={place.groupBy}
